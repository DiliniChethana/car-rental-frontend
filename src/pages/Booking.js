  ...
  const handleBookingSubmit = async () => {
    try {
      ...
      if (!isAuthenticated || !currentUser) {
        ...
        navigate('/login', { state: { from: '/booking', carData, bookingData, personalData } });
        return;
      }
      ...
      const bookingPayload = {
        ...
        userId: currentUser.id || currentUser.username, // Include user ID
      };

      // Try to submit to backend
      console.log('📝 Submitting booking to database...', bookingPayload);
      ...
      if (response.data && response.data.id) {
        ...
        setBookingId(mockId);
        console.log('⚠️ Using mock booking ID:', mockId);
        setError('Booking created locally. Backend integration pending.');
      }
      ...
      if (!error.response) {
        ...
        setError('Cannot connect to server. Please check your internet connection and ensure your backend is running.');
        ...
        setError(`Booking failed: ${error.response.data?.message || 'Unknown error'}`);
      }
      
      // For demo purposes, still create a local booking if there's an error
      const demoId = 'DEMO_' + Date.now();
      setBookingId(demoId);
      setBookingComplete(true);
      setActiveStep(4);
      console.log('📝 Created demo booking with ID:', demoId);
    } finally {
      setLoading(false);
            ...
            <BookingCard>
              <CardContent sx={{ p: 4, textAlign: 'center' }}>
                ...
                <Typography variant="body1" paragraph>
                  ...
                  and saved to your account. You will receive a confirmation email shortly.
                </Typography>
                <Typography variant="body2" color="text.secondary" paragraph>
                  ✅ Your booking has been automatically added to your booking history. 
                  Click "View My Bookings" below to see all your reservations.
                </Typography>
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        <Typography variant="body2">
          <strong>Debug Tools:</strong> If you're experiencing issues, try the{' '}
          <Button 
            component="a" 
            href="/test-connectivity" 
            target="_blank" 
            size="small" 
            variant="outlined"
            sx={{ ml: 1, mr: 1 }}
          ...
          to diagnose backend connection problems.
        </Typography>
      </Alert>
